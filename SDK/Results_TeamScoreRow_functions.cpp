#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Results_TeamScoreRow

#include "Basic.hpp"

#include "Results_TeamScoreRow_classes.hpp"
#include "Results_TeamScoreRow_parameters.hpp"


namespace SDK
{

// Function Results_TeamScoreRow.Results_TeamScoreRow_C.ExecuteUbergraph_Results_TeamScoreRow
// ()
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UResults_TeamScoreRow_C::ExecuteUbergraph_Results_TeamScoreRow(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "ExecuteUbergraph_Results_TeamScoreRow");

	Params::Results_TeamScoreRow_C_ExecuteUbergraph_Results_TeamScoreRow Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.WidgetAnimationEvt_Anim_Absorb_K2Node_WidgetAnimationEvent_0
// (BlueprintEvent)

void UResults_TeamScoreRow_C::WidgetAnimationEvt_Anim_Absorb_K2Node_WidgetAnimationEvent_0()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "WidgetAnimationEvt_Anim_Absorb_K2Node_WidgetAnimationEvent_0");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.Stop Count Down Absorption
// (BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::Stop_Count_Down_Absorption()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "Stop Count Down Absorption");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_1_IntroFinished__DelegateSignature
// (BlueprintEvent)

void UResults_TeamScoreRow_C::BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_1_IntroFinished__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_1_IntroFinished__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_0_CountDownFinished__DelegateSignature
// (BlueprintEvent)

void UResults_TeamScoreRow_C::BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_0_CountDownFinished__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_0_CountDownFinished__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.Count Down Sequence
// (BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::Count_Down_Sequence()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "Count Down Sequence");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_5_CountUpFinished__DelegateSignature
// (BlueprintEvent)

void UResults_TeamScoreRow_C::BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_5_CountUpFinished__DelegateSignature()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "BndEvt__TeamScoreBox_K2Node_ComponentBoundEvent_5_CountUpFinished__DelegateSignature");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.Count Up Sequence
// (BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::Count_Up_Sequence()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "Count Up Sequence");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.Intro Sequence
// (BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::Intro_Sequence()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "Intro Sequence");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.Initialize
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UFortUIScoreReport*               InScoreReport                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// float                                   PlayRate_0                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UResults_TeamScoreRow_C::Initialize(class UFortUIScoreReport* InScoreReport, float PlayRate_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "Initialize");

	Params::Results_TeamScoreRow_C_Initialize Parms{};

	Parms.InScoreReport = InScoreReport;
	Parms.PlayRate_0 = PlayRate_0;

	UObject::ProcessEvent(Func, &Parms);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.SetSkippingToEnd
// (Public, BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::SetSkippingToEnd()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "SetSkippingToEnd");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.SkipToFinalState
// (Public, BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::SkipToFinalState()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "SkipToFinalState");

	UObject::ProcessEvent(Func, nullptr);
}


// Function Results_TeamScoreRow.Results_TeamScoreRow_C.ShowValidResidualScores
// (Public, BlueprintCallable, BlueprintEvent)

void UResults_TeamScoreRow_C::ShowValidResidualScores()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("Results_TeamScoreRow_C", "ShowValidResidualScores");

	UObject::ProcessEvent(Func, nullptr);
}

}

