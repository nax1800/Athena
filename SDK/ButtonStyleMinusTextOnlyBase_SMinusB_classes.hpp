#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: ButtonStyleMinusTextOnlyBase_SMinusB

#include "Basic.hpp"

#include "ButtonStyleMinusTextOnlyBase_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass ButtonStyle-TextOnlyBase_S-B.ButtonStyle-TextOnlyBase_S-B_C
// 0x0000 (0x0730 - 0x0730)
class UButtonStyleMinusTextOnlyBase_SMinusB_C final : public UButtonStyleMinusTextOnlyBase_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"ButtonStyle-TextOnlyBase_S-B_C">();
	}
	static class UButtonStyleMinusTextOnlyBase_SMinusB_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UButtonStyleMinusTextOnlyBase_SMinusB_C>();
	}
};
static_assert(alignof(UButtonStyleMinusTextOnlyBase_SMinusB_C) == 0x000010, "Wrong alignment on UButtonStyleMinusTextOnlyBase_SMinusB_C");
static_assert(sizeof(UButtonStyleMinusTextOnlyBase_SMinusB_C) == 0x000730, "Wrong size on UButtonStyleMinusTextOnlyBase_SMinusB_C");

}

