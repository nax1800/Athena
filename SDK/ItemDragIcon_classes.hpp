#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: ItemDragIcon

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "FortniteUI_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass ItemDragIcon.ItemDragIcon_C
// 0x0018 (0x0250 - 0x0238)
class UItemDragIcon_C final : public UFortUserWidget
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x0238(0x0008)(Transient, DuplicateTransient)
	class UImage*                                 IconImage;                                         // 0x0240(0x0008)(BlueprintVisible, ExportObject, ZeroConstructor, InstancedReference, IsPlainOldData, RepSkip, NoDestructor, PersistentInstance, HasGetValueTypeHash)
	class UTexture2D*                             IconTexture;                                       // 0x0248(0x0008)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)

public:
	void Construct();
	void ExecuteUbergraph_ItemDragIcon(int32 EntryPoint);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"ItemDragIcon_C">();
	}
	static class UItemDragIcon_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UItemDragIcon_C>();
	}
};
static_assert(alignof(UItemDragIcon_C) == 0x000008, "Wrong alignment on UItemDragIcon_C");
static_assert(sizeof(UItemDragIcon_C) == 0x000250, "Wrong size on UItemDragIcon_C");
static_assert(offsetof(UItemDragIcon_C, UberGraphFrame) == 0x000238, "Member 'UItemDragIcon_C::UberGraphFrame' has a wrong offset!");
static_assert(offsetof(UItemDragIcon_C, IconImage) == 0x000240, "Member 'UItemDragIcon_C::IconImage' has a wrong offset!");
static_assert(offsetof(UItemDragIcon_C, IconTexture) == 0x000248, "Member 'UItemDragIcon_C::IconTexture' has a wrong offset!");

}

