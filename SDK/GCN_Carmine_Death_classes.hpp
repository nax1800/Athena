#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: GCN_Carmine_Death

#include "Basic.hpp"

#include "FortniteGame_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass GCN_Carmine_Death.GCN_Carmine_Death_C
// 0x0000 (0x0080 - 0x0080)
class UGCN_Carmine_Death_C final : public UFortGameplayCueNotify_Simple
{
public:
	bool OnActive(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters) const;
	void OnStartParticleSystemSpawned(class UParticleSystemComponent* SpawnedParticleSysComponent, const struct FGameplayCueParameters& Parameters) const;

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"GCN_Carmine_Death_C">();
	}
	static class UGCN_Carmine_Death_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UGCN_Carmine_Death_C>();
	}
};
static_assert(alignof(UGCN_Carmine_Death_C) == 0x000008, "Wrong alignment on UGCN_Carmine_Death_C");
static_assert(sizeof(UGCN_Carmine_Death_C) == 0x000080, "Wrong size on UGCN_Carmine_Death_C");

}

